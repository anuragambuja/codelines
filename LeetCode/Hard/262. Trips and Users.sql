...
select a.request_at day, round(coalesce(b.ctot/a.tot, 0),2) "Cancellation Rate" from
(select request_at, count(1) tot from trips t, users u
where t.client_id = u.users_id and u.banned = 'No' and t.Request_at between '2013-10-01' and '2013-10-03' 
group by t.request_at) a
left join
(select request_at, count(1) ctot from trips t, users u
where t.client_id = u.users_id and u.banned = 'No' and t.Request_at between '2013-10-01' and '2013-10-03'
