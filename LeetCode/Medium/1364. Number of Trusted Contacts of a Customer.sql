...
select a.invoice_id, max(c.customer_name) customer_name, max(a.price) price, count(b.contact_email) contacts_cnt, sum(case when b.contact_email in (select distinct email from customers) then 1 else 0 end) trusted_contacts_cnt 
from invoices a 
left join contacts b
on a.user_id = b.user_id
left join customers c
on a.user_id = c.customer_id
group by a.invoice_id, a.user_id
